...
//
//  Changes from the original "usb.c":
//    Call the Canon specific function, _canon_bj(), from the main().
//

/*
 * "$Id: canon_usb.c,v 1.2 2004/12/07 06:59:47 teru Exp $"
 *
 ...
 */
	if ((strncmp(argv[0], "canon_usb:/dev", 14) == 0))
	{
		memset(resource, 0, 1024);
		strncpy(resource, argv[0] + 10, 1023);
	}
	else
	{
  ...
  */
  ...
  if ((probe = fopen("/proc/bus/usb/devices", "r")) != NULL)
  {
	...
	*/
	...
	printf("direct canon_usb:%s \"%s %s\" \"USB Printer #%d with status readback for Canon BJ\"\n",
    ...
    */
	...
	if (access(device, 0))
	{
      ...
      printf("direct canon_usb:%s \"Unknown\" \"USB Printer #%d with status readback for Canon BJ\"\n", device, i + 1);
    }
  }
  else
  {
